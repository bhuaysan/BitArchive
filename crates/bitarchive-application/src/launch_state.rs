...
//! an OS API, so none of these types names one. A missing runtime is
//! [`LaunchRuntimeResolution::Missing`], not a concrete store error; a core is an
//! identity plus two paths, not a `ManagedCore`. The adapter that knows how to read
//! a store translates its own failures into these values, and that translation is
//! the only place the two vocabularies meet.
//!
//! # What can fail, and what is merely absent
//!
//! A negotiation never fails. Every fact it needs is either present or absent, and
//! an absence becomes a [`LaunchBlocker`](crate::game_launch::LaunchBlocker):
//!
//! ```text
//! runtime   resolved | missing
//! core      curated + installed, curated but not installed, not curated, unavailable
//! firmware  present | missing      (per requirement, with the level deciding)
//! content   available | missing
//! ```
//!
//! Distinguishing "not installed" from "installed but unusable" is deliberate: the
//! first is repaired by installing the component, the second is a broken
//! installation that needs diagnosis. A caller that received one generic "no core"
//! answer could not tell a first run from a corrupt store.
...
impl LaunchRuntimeResolution {
    ...
    /// Returns the identity of the runtime that could not be resolved.
    ///
    /// A resolved runtime carries its own identity, so "which runtime is this
    /// about?" has an answer in both states.
    #[must_use]
    pub fn id(&self) -> &str {
        match self {
...
mod tests {
    ...
    /// A runtime resolution either holds a runtime or names the missing one, and
    /// never both.
    #[test]
    fn a_runtime_resolution_is_either_resolved_or_missing() {
        let resolved = LaunchRuntimeResolution::Resolved(LaunchRuntime::new(
            "retroarch",
            "1.22.2",
            "/components/runtime/retroarch/RetroArch",
        ));
        let missing = LaunchRuntimeResolution::Missing {
